...
function getSide(type: string): "BACK" | "LAY" {
  ...
  if (type === "yes")  return "BACK";  // LINE market Yes → BACK
  if (type === "no")   return "LAY";   // LINE market No  → LAY
  return "BACK";
}
// ────────────────────────────────────────────────────────────────
...
export default function MBetSlip({ onPreviewChange }: MBetSlipProps) {
  ...
  const { selectedBet, clearSelectedBet, stakeValue, setUserBalance } = useAppStore();
...
useEffect(() => {
  ...
  const accentVar =
    type === "yes" ? "#50d0ae"
    : type === "no" ? "#5baca7"
    : isBack ? "var(--bs-back-accent)"
    : "var(--bs-lay-accent)";

  const accentBg15 =
    type === "yes" ? "#50d0ae26"
    : type === "no" ? "#5baca726"
    : isBack ? "var(--bs-back-accent-bg15)"
  ...
  const headerLabel =
    type === "back" ? "Back (Bet For)"
    : type === "lay" ? "Lay (Bet Against)"
    : type === "yes" ? "Yes (Bet For)"
    : "No (Bet Against)";
  ...
  // ── Quick stake: adds the amount (same as T20 handleQuickAmount) ─
  const handleQuickValues = (amount: string) => {
    const add = Number(amount) || 0;
    setStake((prev) => Number(prev || 0) + add);
  };

  // ── Place Bet — same flow as T20 DBetslip ─────────────────────
  const handlePlaceBet = async () => {
    ...
    try {
      ...
      if (ok) {
        showToast(msg.status, msg.title, msg.desc || "Bet placed successfully.");
        clearSelectedBet();
        setStake(0);
        ...
        try {
          ...
          if (balRes?.data?.data) {
            setUserBalance(balRes.data.data);
          }
        } catch { /* silent */ }
        ...
        showToast("error", msg.title || "Failed", msg.desc || "Please try again.");
      }
    } catch (err: any) {
      const raw = err?.response?.data?.meta?.message || err?.message || "Network error.";
...
const isPlaceDisabled = placing || stake < MIN_STAKE || odds <= 1;
        ...
        <div className="p-4 flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-1.5">
              <label className="text-[11px] ml-2.5" style={{ color: "var(--bs-label)" }}>
              ...
              <div className="flex items-center rounded-full px-1 py-1 gap-1" style={{ background: "var(--bs-input-bg)", border: "1px solid var(--bs-odds-border)" }}>
                <button type="button" onClick={handleDecrease} className="bs-circle-btn w-7 h-7 shrink-0 rounded-full flex items-center justify-center cursor-pointer" style={{ background: "var(--bs-circle-btn-bg)", color: "var(--bs-text)" }}>
                ...
                <button type="button" onClick={handleIncrease} className="bs-circle-btn w-7 h-7 shrink-0 rounded-full flex items-center justify-center cursor-pointer" style={{ background: "var(--bs-circle-btn-bg)", color: "var(--bs-text)" }}>
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-1.5">
              ...
              <div className="flex items-center rounded-full px-1 py-1 gap-1" style={{ background: "var(--bs-input-bg)", border: `1px solid ${accentVar}` }}>
                <button type="button" onClick={handleDecreaseStake} className="bs-circle-btn w-7 h-7 shrink-0 rounded-full flex items-center justify-center cursor-pointer" style={{ background: accentBg15, color: accentVar }}>
                  <Minus className="w-3 h-3" />
                </button>
                ...
                <button type="button" onClick={handleIncreaseStake} className="bs-circle-btn w-7 h-7 shrink-0 rounded-full flex items-center justify-center cursor-pointer" style={{ background: accentBg15, color: accentVar }}>
                  <Plus className="w-3 h-3" />
                </button>
              </div>
            </div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {quickValues.map((value, i) => (
              <button
                ...
                style={{ background: "var(--bs-quick-pill-bg)", border: "1px solid var(--bs-quick-pill-border)", color: "var(--bs-text)" }}
              >
                {value}
              </button>
            ))}
  ...
  disabled={isPlaceDisabled}
              ...
              style={{ background: accentVar, cursor: (stake === 0 || placing) ? "not-allowed" : "pointer", opacity: (stake === 0 || placing) ? 0.45 : 1 }}
            >
              {placing ? "Placing…" : (
                <>
                  Place Bet
