      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed left-0 w-full min-[960px]:hidden z-[999999] overflow-hidden"
            style={{
              top: `${MOBILE_TOP_OFFSET}px`,
              height: `calc(100dvh - ${MOBILE_TOP_OFFSET}px)`,
              WebkitBackdropFilter: "blur(20px)",
          ...
          {/* DESKTOP */}
          <motion.div
            key="exposure-desktop"
            initial={{ x: "-50%", y: "calc(-100% - 40px)" }}
            animate={{ x: "-50%", y: 0 }}
            exit={{ x: "-50%", y: "calc(-100% - 40px)" }}
            transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }}
            className="fixed hidden min-[960px]:block z-[999999]"
            style={{
              top: `${DESKTOP_TOP_OFFSET}px`,
              left: "50%",
              width: "min(710px, calc(100vw - 40px))",
