...
<div className="w-[4px] h-[4px] min-[500px]:w-[5px] min-[500px]:h-[5px] 
                bg-[#ffbbbb] 
...
<ul className="grid grid-cols-3 gap-3 list-none px-2 m-0 max-[350px]:gap-2">
  {filteredItems?.map((item: any, index: number) => (
    <li key={index} className="w-full">
      <article
        className="relative w-full aspect-[2/2.5] min-h-[166px] rounded-xl overflow-hidden shadow-2xl transition-transform duration-300 [transform:perspective(1000px)_rotateY(5deg)_skewX(-5deg)] border border-white/10 cursor-pointer group"
      >
        {/* Background Image */}
        <div
        ...
        <div className="absolute bottom-0 left-0 w-full p-2 flex flex-col items-center justify-end h-full">
          <h3 className="text-white text-sm font-bold uppercase text-center mb-1 tracking-wide drop-shadow-md">
